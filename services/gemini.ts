...
let currentTmdbApiKey = '1865f43a0549ca50d341dd9ab8b29f49';
...
const MAIN_FEED_CONFIGS: RowConfig[] = [
  ...
  {
    title: 'Popular on Cinemeta (Movies)',
  ...
  {
    title: 'Popular on Cinemeta (Series)',
  ...
  },
  {
    ...
    endpoint: '/discover/movie',
    params: { with_genres: '18', 'vote_average.gte': '7' },
    mediaType: MediaType.MOVIE,
  },
  {
  ...
  },
  {
    title: 'War & Politics',
    endpoint: '/discover/tv',
    params: { with_genres: '10768', sort_by: 'popularity.desc' },
    mediaType: MediaType.TV_SHOW,
  },
  {
    title: 'Westerns',
    endpoint: '/discover/movie',
    params: { with_genres: '37', sort_by: 'popularity.desc' },
    mediaType: MediaType.MOVIE,
  },
  {
    title: 'Reality TV',
...
const transformTmdbItem = (item: any, mediaType?: MediaType): MediaItem => {
  ...
  const title =
    item.title || item.name || item.original_title || 'Unknown Title';
...
const createDynamicRowConfig = (
  ...
  const targetType =
    typeFilter || (Math.random() > 0.4 ? MediaType.MOVIE : MediaType.TV_SHOW);
  const endpoint =
    targetType === MediaType.MOVIE ? '/discover/movie' : '/discover/tv';
    ...
    if (genreId && genreName) {
      ...
      if (!typeFilter || typeFilter === MediaType.MOVIE) {
        baseConfigs.push(
          ...
          {
            title: `Top Rated ${genreName} Movies`,
            endpoint: '/discover/movie',
            params: {
              with_genres: String(genreId),
          ...
          {
            title: `Classic ${genreName} Movies`,
            endpoint: '/discover/movie',
            params: {
              with_genres: String(genreId),
      ...
      if (!typeFilter || typeFilter === MediaType.TV_SHOW) {
        baseConfigs.push(
          {
            title: `Trending ${genreName} Shows`,
            endpoint: '/discover/tv',
            params: {
              with_genres: String(genreId),
              sort_by: 'popularity.desc',
          ...
          {
            title: `Critically Acclaimed ${genreName} Series`,
            endpoint: '/discover/tv',
            params: {
              with_genres: String(genreId),
              sort_by: 'vote_average.desc',
      ...
      if (typeFilter) {
        staticConfigs = MAIN_FEED_CONFIGS.filter(
          (c) => c.mediaType === typeFilter,
        );
      } else {
    ...
    const promises = batchConfigs.map(async (config) => {
      try {
        ...
        if (config.source === 'cinemeta') {
          ...
          items = data.results.map((i: any) =>
            transformTmdbItem(i, config.mediaType),
          );
        }
...
export const searchMediaCatalog = async (
  query: string,
): Promise<MediaItem[]> => {
  ...
  try {
    ...
    const tmdbPromise = (async () => {
      ...
      if (yearMatch) {
        ...
        const m = movies.results.map((i: any) =>
          transformTmdbItem(i, MediaType.MOVIE),
        );
        const t = tv.results.map((i: any) =>
          transformTmdbItem(i, MediaType.TV_SHOW),
        );
        return [...m, ...t];
      }
    ...
    const [tmdbResults, cinemetaResults] = await Promise.all([
      tmdbPromise,
      cinemetaPromise,
    ]);
...
export const getMediaDetails = async (
  id: string,
  typeHint: MediaType = MediaType.MOVIE,
  ...
  try {
    ...
    if (isImdbId) {
      const find = await fetchTmdb(`/find/${id}`, {
...
export const getSeasonEpisodes = async (
  ...
  season: number,
): Promise<{ episodes: Episode[]; seasonPoster?: string }> => {
